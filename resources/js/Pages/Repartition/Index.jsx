...
function Toast({ flash, t }) {
    ...
    const map = {
        repartition_saved:      t('repartitionSaved'),
        repartition_no_changes: t('repartitionNoChanges'),
    };
    useEffect(() => {
...
function RepartitionContent({ filieres, niveaux, semestres, modules, salles, students, totalCapacite, filters, t, locale, isRTL }) {
    ...
    const applyFilters = (overrides = {}) => {
        ...
        router.get(route('repartition.index'), params, { preserveState: true, replace: true });
    };
    ...
    // Auto-distribute: round-robin fill by capacity
    ...
    const save = () => {
        ...
        router.post(route('repartition.save'), { module_id: moduleId, Nexam: parseInt(nexam), repartition }, {
            preserveState: true,
            preserveScroll: true,
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-900 p-4 sm:p-6 lg:p-8">
            <div className="mx-auto max-w-7xl">
                {/* Header */}
                <div className="mb-6">
                    <h1 className="text-2xl font-bold text-slate-800 dark:text-white">{t('repartitionTitle')}</h1>
                    <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{t('repartitionSubtitle')}</p>
                </div>
...
export default function RepartitionIndex({ filieres, niveaux, semestres, modules, salles, students, totalCapacite, filters }) {
    ...
    return (
        <LanguageProvider>
            <AdminLayout>
                ...
                <RepartitionContent {...{ filieres, niveaux, semestres, modules, salles, students, totalCapacite, filters, t, locale, isRTL }} />
            </AdminLayout>
        </LanguageProvider>
    );
}
