    ...
    return (
        <div className={`flex items-center gap-4 rounded-2xl border border-slate-200 dark:border-slate-700/60 ${c.bg} px-5 py-4`}>
            <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl ${c.ring}`}>
                <Icon icon={icon} className={`h-5 w-5 ${c.icon}`} />
            </div>
            <div>
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="mt-1 flex flex-wrap items-center gap-1.5">
                                            ...
                                            {data.sexe && (
                                                <span className="text-[10px] text-slate-400 flex items-center"><Icon icon={data.sexe === 'M' ? ICONS.male : ICONS.female} className="h-3.5 w-3.5" /></span>
                                            )}
                                        </div>
                                    </div>
                                </div>
                        ...
                        {!file && (
                            ...
                            >
                                ...
                                <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-slate-200 dark:bg-slate-700 group-hover:bg-indigo-100 dark:group-hover:bg-indigo-900/30 transition">
                                    <ArrowUpTrayIcon className="h-6 w-6 text-slate-400 group-hover:text-indigo-500 transition" />
                                </div>
                                ...
                                <>
                                    ...
                                    <button type="button" onClick={submit}
                                        ...
                                        {status === 'loading' ? (
                                            ...
                                            <><ArrowUpTrayIcon className="h-4 w-4" />{t('importStart')}</>
                                        )}
                                    </button>
                                </>
                            )}
                                    ...
                                    <div className="space-y-1">
                                        {selectedFields.map((key, idx) => (
                                            <div key={key} draggable
                                                ...
                                                <span className="text-slate-400"><Bars3BottomLeftIcon className="h-4 w-4" /></span>
                    ...
                    <div className={`shrink-0 border-t border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 px-6 py-4 flex items-center justify-between gap-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
                        ...
                        <button onClick={doExport} disabled={loading || selectedFields.length === 0}
                            ...
                            {loading ? (
                                ...
                                <><ArrowDownTrayIcon className="h-4 w-4" />{t('exportDownload')}</>
                            )}
                        </button>
                    </div>
                </div>
                    ...
                    <div className="flex items-center gap-2.5 min-w-0">
                        ...
                        <div className="min-w-0">
                            ...
                            <div className="mt-0.5 flex items-center gap-1.5">
                                {etudiant.sexe === 'M' && <span className="flex items-center text-indigo-500"><UserIcon className="h-3.5 w-3.5" /></span>}
                                {etudiant.sexe === 'F' && <span className="flex items-center text-rose-500"><UserIcon className="h-3.5 w-3.5" /></span>}
                                {etudiant.CNE && (
            ...
            <td className="px-5 py-3.5">
                {etudiant.sexe
                    ? <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-semibold ${
                        ...
                        <Icon icon={etudiant.sexe === 'M' ? ICONS.male : ICONS.female} className="h-3.5 w-3.5 shrink-0" />
    ...
    return (
        <div className="flex flex-col items-center justify-center py-24 text-center">
            <div className="mb-5 flex h-20 w-20 items-center justify-center rounded-2xl bg-slate-100 dark:bg-slate-800">
                <Icon icon={hasFilter ? MagnifyingGlassIcon : UserGroupIcon} className="h-9 w-9 text-slate-300 dark:text-slate-600" />
            </div>
            ...
            {!hasFilter && (
                <button onClick={onAdd}
                    ...
                    <PlusIcon className="h-4 w-4" />{t('addEtudiant')}
                </button>
            )}
        </div>
    );
                    ...
                    <div>
                        <h1 className="flex items-center gap-2.5 text-xl font-bold text-slate-800 dark:text-white">
                            <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-indigo-100 dark:bg-indigo-900/30">
                                <AcademicCapIcon className="h-4 w-4 text-indigo-600 dark:text-indigo-400" />
                            </span>
                    ...
                    <div className={`flex flex-wrap items-center gap-2 -mt-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                        ...
                        {search && (
                            <span className="flex items-center gap-1 rounded-full bg-indigo-100 dark:bg-indigo-900/30 px-2.5 py-0.5 text-xs font-medium text-indigo-700 dark:text-indigo-300">
                                "{search}"
                                <button onClick={() => handleSearch('')} className="ms-1 hover:text-red-500"><XMarkIcon className="h-3 w-3" /></button>
                            </span>
                        )}
                        {sexeFilter && (
                            <span className={`flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium ${
                                ...
                                <Icon icon={sexeFilter === 'M' ? ICONS.male : ICONS.female} className="h-3.5 w-3.5 shrink-0" />
                                {sexeFilter === 'M' ? t('etudiantSexeM') : t('etudiantSexeF')}
                                <button onClick={() => handleSexe('')} className="ms-1 hover:text-red-500"><XMarkIcon className="h-3 w-3" /></button>
                            </span>
                        )}
                        {filiereIdFilter && (
                            <span className="flex items-center gap-1 rounded-full bg-indigo-100 dark:bg-indigo-900/30 px-2.5 py-0.5 text-xs font-medium text-indigo-700 dark:text-indigo-300">
                                {filieres?.find(f => f.id == filiereIdFilter)?.code || filiereIdFilter}
                                <button onClick={() => handleFiliereId('')} className="ms-1 hover:text-red-500"><XMarkIcon className="h-3 w-3" /></button>
                            </span>
                        )}
                        {niveauFilter && (
                            <span className="flex items-center gap-1 rounded-full bg-violet-100 dark:bg-violet-900/30 px-2.5 py-0.5 text-xs font-medium text-violet-700 dark:text-violet-300">
                                {niveaux?.find(n => n.id == niveauFilter)
                                    ...
                                    : niveauFilter}
                                <button onClick={() => handleNiveau('')} className="ms-1 hover:text-red-500"><XMarkIcon className="h-3 w-3" /></button>
                            </span>
                        )}
